CLASS zcl_generate_travel_data_121 DEFINITION
 PUBLIC
 FINAL
 CREATE PUBLIC .
...
CLASS ZCL_GENERATE_TRAVEL_DATA_121 IMPLEMENTATION.

 METHOD if_oo_adt_classrun~main.
   DATA:itab TYPE TABLE OF ztravel_121.
...
*   delete existing entries in the database table
   DELETE FROM ztravel_121.

*   insert the new table entries
   INSERT ztravel_121 FROM TABLE @itab.

*   check the result
   SELECT * FROM ztravel_121 INTO TABLE @itab.
   out->write( sy-dbcnt ).
